...
qq는 설계부터 배포까지 전체 워크플로우를 커버하는 26개의 슬래시 커맨드를 제공한다: `/qq:design` → `/qq:plan` → `/qq:execute` → `/qq:test` → `/qq:codex-code-review` → `/qq:commit-push`. 이 접근법은 [AI Coding in Practice: An Indie Developer's Document-First Approach](https://tyksworks.com/posts/ai-coding-workflow-en/)에서 설명한 문서 우선 방법론에 기반한다.
...
- macOS 또는 Windows(Windows 는 bash 와 코어 유틸리티를 위해 [Git for Windows](https://gitforwindows.org/) 필요)
...
**Windows에서 동작하나요?**
네. [Git for Windows](https://gitforwindows.org/)가 필요합니다(bash, curl 및 핵심 유틸리티 제공). 1.15.x 와 1.16.x 시리즈에서 Windows 지원이 크게 강화되었습니다 — LF 줄바꿈 강제, 경로 정규화, Python Store 별칭 감지, hook 스크립트의 jq 폴백.
