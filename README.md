...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-v1.16.17-blue" alt="Version">
...
quick-question 就是把这个落差闭环起来的运行时层。它把四个工作模式——`prototype`（原型）、`feature`（功能）、`fix`（修复）、`hardening`（加固）——当成一等公民状态，而不是装饰文字。原型保持编译绿灯、保持可玩；加固阶段在交付前强制要求测试、审阅和文档/代码一致性。artifact 驱动的控制器（`/qq:go`）读取 `.qq/state/*.json` 和你的 `work_mode`，然后推荐具体的下一个 skill——而不是从聊天历史里瞎猜。

运行时是引擎对称、agent 无关的。tykit 给 Unity 提供最深度的集成（进程内 HTTP 服务器，毫秒级响应）。Godot、Unreal、S&box 通过 Python 桥接达到运行时对等。[Claude Code](https://docs.anthropic.com/en/docs/claude-code) 拿到 26 个 skill、自动编译 hook 和审阅门。Codex、Cursor、Continue 以及任何 MCP 兼容宿主通过 HTTP 和 MCP 拿到同一个底层运行时。`.qq/` 中的结构化状态是磁盘上的纯 JSON——任何 agent 都能跨会话读取。

方法论基于 [《AI 编程实践：独立开发者的文档驱动方法》](https://tyksworks.com/posts/ai-coding-workflow-zh/) 中描述的 document-first 方法。
...
- **Hooks** 在每次 Claude Code 工具调用时触发。PostToolUse 在 `Edit`/`Write` 之后通过 `qq-compile.sh`（多引擎调度器）编译引擎源文件。PreToolUse 在审阅验证期间或编译红灯时阻止编辑，直到对应门解除。Stop 在 skill 被修改但未运行 `/qq:self-review` 时阻止会话退出。所有门控/临时文件按 `$PPID` 隔离 session。
- **控制器** —— `/qq:go` 读取 `.qq/state/*.json`、最近的运行记录、你的 `work_mode` 和当前 `policy_profile`，然后路由到下一个 skill。它是控制器，不是实现引擎——只在状态模糊时才退回到上下文启发式判断。
- **引擎桥接** —— tykit（Unity 进程内 HTTP）、`godot_bridge.py`、`unreal_bridge.py`、`sbox_bridge.py`。编译、测试、控制台、find / inspect——都是经过验证的执行，不是盲目的文件写入。
- **运行时数据** —— `.qq/runs/*.json`（原始运行日志）、`.qq/state/*.json`（最新编译 / 测试状态）、`.qq/state/session-decisions.json`（跨 skill 决策日志）、`.qq/telemetry/`。纯 JSON，任何 agent 跨会话可读。

代码审阅有两种对称模式共享同一个验证循环：**Codex 审阅**（`code-review.sh` → `codex exec`，跨模型）和 **Claude 审阅**（`claude-review.sh` → `claude -p`，单模型）。两者都产出发现，然后独立的验证子 agent 对照实际源码逐条检查并标记过度设计——最多 5 轮直到通过。统一的审阅门（`scripts/hooks/review-gate.sh`）在所有验证子 agent 完成之前阻止编辑（三字段格式：`<ts>:<completed>:<expected>`）。MCP 为非 Claude 宿主提供一次性 `qq_code_review` 和 `qq_plan_review` 工具。

层级细节参见[架构总览](docs/dev/architecture/overview.md)、[Hook 系统](docs/zh-CN/hooks.md)、[跨模型审阅](docs/zh-CN/cross-model-review.md)和[并行 Worktree](docs/zh-CN/worktrees.md)。
...
- `python3`（必需）、`jq`（推荐——hook 脚本在 jq 缺失时会回退到 python3）
...
所有 agent 共享同一套运行时安装。它会自动检测引擎，并把脚本、桥接和配置写入项目。
...
| **Cursor / Continue / 其他 MCP 宿主** | 在宿主配置的 `mcpServers` 中加 `qq`：`command: python3`、`args: ["./scripts/qq_mcp.py"]`、`cwd: /path/to/project` | 编译、测试、控制台、编辑器控制都作为 MCP tool——参见 [`docs/zh-CN/tykit-mcp.md`](docs/zh-CN/tykit-mcp.md) |
| **任何 HTTP 客户端** | 从 `Temp/tykit.json` 读取端口，向 `localhost:$PORT/` POST JSON | 直接访问 tykit——参见 [`docs/zh-CN/tykit-api.md`](docs/zh-CN/tykit-api.md) |
...
```bash
/qq:go                                       # 检测阶段，推荐下一步
/qq:design "拖放式背包系统"                  # 写设计文档
/qq:plan                                     # 生成实现计划
/qq:execute                                  # 实现代码，每次编辑自动编译
/qq:test                                     # 运行测试，捕获运行时错误
/qq:commit-push                              # 批量提交并推送
```

`/qq:go` 会根据你的 `work_mode` 调整。`prototype` 模式下保持轻量（编译绿灯、可运行）。`hardening` 模式下在交付前强制要求测试、审阅、文档 / 代码一致性。详细演练参见[快速上手](docs/zh-CN/getting-started.md)。
...
|------|------|
| `/qq:go` | 检测工作流阶段，推荐下一步 |
| `/qq:bootstrap` | 把游戏愿景分解成 epics，编排完整 pipeline |
| `/qq:design` | 编写游戏设计文档 |
| `/qq:plan` | 生成技术实现计划 |
...
| `/qq:claude-code-review` | 单 Claude 深度代码审阅，自动修复循环 |
| `/qq:claude-plan-review` | 单 Claude 计划 / 设计审阅 |
...
| `/qq:tech-research` | 在开源社区搜索技术解决方案 |
| `/qq:design-research` | 搜索游戏设计参考和模式 |
...
| 模式 | 适用场景 | 必须做 | 通常跳过 |
|------|---------|--------|---------|
| `prototype` | 新玩法、灰盒、fun check | 保持编译绿灯，保持可运行 | 正式文档，完整审阅 |
| `feature` | 构建可保留的系统 | 设计、计划、编译、定向测试 | 每次改动跑完整回归 |
| `fix` | bug 修复、回归修复 | 先复现，最小安全修复 | 大规模重构 |
| `hardening` | 风险重构、发版前 | 测试、审阅、文档 / 代码一致性 | 原型快捷方式 |

在 `qq.yaml` 中设置共享默认值。在 `.qq/local.yaml` 中按 worktree 覆盖。`work_mode` 和 `policy_profile` 是两个独立的旋钮：前者回答"这是什么类型的任务？"，后者回答"这个项目需要多少验证？"。原型和加固阶段可以共享同一个 policy profile，也可以不共享。完整参考参见[配置](docs/zh-CN/configuration.md)。
...
**支持 Windows 吗？**
支持。需要 [Git for Windows](https://gitforwindows.org/) 提供 bash、curl 和核心工具。1.15.x 和 1.16.x 系列对 Windows 支持做了大量加固——通过 `.gitattributes` 强制 LF 行结束、路径规范化、Python Store alias 检测，以及 hook 脚本里的 jq fallback。

**必须装 Codex CLI 吗？**
不必。Codex CLI 启用跨模型审阅（`/qq:codex-code-review`），但 `/qq:claude-code-review` 不需要它就能用。

**能和 Cursor / Copilot / Codex / 其他 agent 一起用吗？**
可以。运行时层（tykit、引擎桥接、`.qq/` 状态、脚本）是 agent 无关的——任何能发 HTTP 或 MCP 的工具都可以使用。26 个 `/qq:*` slash 命令和自动编译 hook 是 Claude Code 专属的，但底层调用的脚本是普通的 shell 和 Python。参见 [`docs/dev/agent-integration.md`](docs/dev/agent-integration.md)。

**编译失败了会怎样？**
自动编译 hook 会捕获错误输出并显示在对话中。编译门会阻止后续对引擎源文件的编辑，直到编译恢复。agent 读取错误信息并修复代码，hook 会自动重新编译。

**能不装 quick-question 单独用 tykit 吗？**
可以。把 [`packages/com.tyk.tykit/`](packages/com.tyk.tykit/) 中的 UPM 包加到项目即可。参见 [tykit README](packages/com.tyk.tykit/README.md)。

**支持哪些 Unity 版本？**
tykit 需要 Unity 2021.3+。MCP 替代方案：[mcp-unity](https://github.com/nicoboss/mcp-unity) 需要 Unity 6+；[Unity-MCP](https://github.com/mpiechot/Unity-MCP) 没有版本要求。
